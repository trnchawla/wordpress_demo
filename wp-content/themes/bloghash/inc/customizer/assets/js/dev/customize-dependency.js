;( function( $ ) {
	...
	var bloghashDependencies = {
		...
		init: function() {
			...
			wp.customize.control.each( function( control ) {
				self.showBloghashControl( control );
			});
					...
					wp.customize( master, function( setting ) {
						var setupControl = function( control ) {
							...
							isDisplayed = function() {
								return self.showBloghashControl( wp.customize.control( slave ) );
							};
		 ...
		 * @returns {bool}
		 */
		showBloghashControl: function( control ) {
			var self     = this,
				show     = true,
				i;
		 ...
		 *
		 * @since 1.0.0
		 * @param {Object} requirement - The requirement, inherited from showBloghashControl.
	...
	}; // END var bloghashDependencies

	$( document ).ready( function() {
		bloghashDependencies.init();
	});
