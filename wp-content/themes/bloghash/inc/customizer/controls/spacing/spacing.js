;(function($) {
 	...
 	wp.customize.controlConstructor['bloghash-spacing'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			// Change unit.
			control.container.find( '.bloghash-control-unit input[type="radio"]' ).on( 'change', function() {
				control.save_value();
			});
		...
		save_value: function() {
			...
			var new_val = {},
				...
				choice,
				device,
				units   = this.container.find( '.bloghash-control-unit' );
