;(function( $ ) {
	...
	wp.customize.controlConstructor['bloghash-button'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.on( 'click', '.button', function(e){
				...
				if ( action ) {
					...
					var data = {
						_ajax_nonce: bloghash_customizer_localized.wpnonce,
						action: action,
					};

			 		$.post( bloghash_customizer_localized.ajaxurl, data, function(response) {
