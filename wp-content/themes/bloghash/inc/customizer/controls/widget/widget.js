;(function($) {
	...
	wp.customize.controlConstructor['bloghash-widget'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.on( 'click', '.bloghash-widget-container .widget-top', function(){
				$(this).closest( '.widget' ).toggleClass( 'bloghash-expanded' ).find( '.widget-inside' ).slideToggle( 200 );
			});
			
			// Minimize widget content when clicked on Done
			control.container.on( 'click', '.bloghash-widget-container .widget-control-close', function() {
				$(this).closest( '.widget' ).toggleClass( 'bloghash-expanded' ).find( '.widget-inside' ).slideToggle( 200 );
			});

			// Show available widgets
			control.container.on( 'click', '.bloghash-add-widget', function(e) {
				e.preventDefault();
				control.updateList();
			});
		 	...
		 	control.container.on( 'click', '.bloghash-widget-edit-nav', function(){
		 		wp.customize.control( 'nav_menu_locations[' + $(this).closest( '.bloghash-widget-nav-container' ).data( 'menu-location' ) + ']' ).focus();
		 		control.close();
		 	});
			...
			$( control.container ).find( '.bloghash-widget-nav-container' ).each( function(){
			...
			wp.customize( 'nav_menu_locations[' + menu_location + ']', function( value ) {
				value.bind( function( newval ) {
					...
					if ( newval ) {
						...
						$container.addClass( 'bloghash-widget-nav-has-menu' )
							.find( '.bloghash-widget-nav-name' )
							.html( menu_name );
					} else {
						$container.removeClass('bloghash-widget-nav-has-menu');
					}
		...
		updateList: function(){
			...
			if ( widgets ) {
				...
				// Hide all widgets.
				$( '#bloghash-available-widgets-list .bloghash-widget' ).hide().removeClass('disabled');

				// Display allowed widgets.
				$.each( widgets, function( index, el ) {

					widget = $( '#bloghash-available-widgets-list #bloghash-widget-tpl-bloghash_customizer_widget_' + index );

					widget.show();

					if ( el.hasOwnProperty( 'max_uses' ) && el.max_uses > 0 && el.max_uses <= $(self.container).find('.bloghash-widget-container [data-widget-type="' + index + '"]').length ) {
						widget.addClass('disabled');
					}
				});
			} else {
				// Show all widgets
				$( '#bloghash-available-widgets-list .bloghash-widget' ).show();
			}
		},
		...
		addWidget: function( widget_id_base ) {
			...
			// Get widget form
			widget_html = $.trim( $(this.container).find( '.bloghash-widget-tpl-' + widget_id_base ).html() );
			widget_html = widget_html.replace( /<[^<>]+>/g, function( m ) {
				return m.replace( /__i__|%i%/g, widget_uuid );
			} );

			// Append new widget.
			var $widget = $( widget_html ).appendTo( this.container.find( '.bloghash-widget-container' ) );
			...
			if ( $widget.find( '.bloghash-widget-nav-container' ).length ) {
				this.bindMenuLocation( $widget.find( '.bloghash-widget-nav-container' ) );
			}
		},

		close: function() {
			$( 'body' ).removeClass( 'bloghash-adding-widget' );
		},

		update: function() {

			// Get all widgets in the area
			var widgets = this.container.find( '.bloghash-widget-container .widget' );
		...
		setupSortable: function() {
			...
			$( this.container ).find( '.bloghash-widget-container' ).sortable({
				items: '> .widget',
				handle: '.widget-top',
				intersect: 'pointer',
				axis: 'y',
 	...
 	$(document).ready( function(){
	 	...
	 	$( '.wp-full-overlay' ).on( 'click', '.bloghash-add-widget, .bloghash-close-widgets-panel', function(e) {
	 		e.preventDefault();

	 		$( 'body' ).toggleClass( 'bloghash-adding-widget' );

	 		if ( $( this ).data( 'location-title' ) ) {
	 			control = wp.customize.control( $(this).data('control') );
	 			$( '#bloghash-available-widgets' ).attr( 'data-control', control.params.id ).find( '.bloghash-widget-caption' ).find( 'h3' ).html( $(this).data( 'location-title' ) );
	 		}
	 	});

	 	$( '.wp-full-overlay' ).on( 'click', '.customize-section-back', function(e) {
	 		$( 'body' ).removeClass( 'bloghash-adding-widget' );
	 		$( '#bloghash-available-widgets' ).removeAttr( 'data-control' );
	 	});

		// Add widget to widget control.
	 	$( '#bloghash-available-widgets' ).on( 'click', '.bloghash-widget', function(e) {

	 		// Get active control.
			control = wp.customize.control( $( '#bloghash-available-widgets' ).attr('data-control') );
