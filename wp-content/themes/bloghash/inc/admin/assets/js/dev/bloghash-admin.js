//--------------------------------------------------------------------//
// Bloghash script that handles our admin functionality.
//--------------------------------------------------------------------//
...
;( function( $ ) {
	...
	var BloghashAdmin = {
		...
		init: function() {
			...
			// Document ready
			$( document ).ready( BloghashAdmin.ready );

			// Window load
			$( window ).on( 'load', BloghashAdmin.load );

			// Bind UI actions
			BloghashAdmin.bindUIActions();

			// Trigger event when Bloghash fully loaded
			$( document ).trigger( 'bloghashReady' );
		},
		...
		*/
		bindUIActions: function() {
			...
			$wrap.on( 'click', '.plugins .hester-btn:not(.active)', function( e ) {
				...
				BloghashAdmin.pluginAction( $this );
			});

			$( document ).on( 'wp-plugin-install-success', BloghashAdmin.pluginInstallSuccess );
			$( document ).on( 'wp-plugin-install-error',   BloghashAdmin.pluginInstallError );
		},
		...
		pluginInstallSuccess: function( event, response ) {
			...
			BloghashAdmin.pluginAction( $button );
		},
	...
	}; // END var BloghashAdmin

	BloghashAdmin.init();
	window.bloghashadmin = BloghashAdmin;
